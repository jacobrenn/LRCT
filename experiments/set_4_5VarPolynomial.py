...
if __name__ == '__main__':
    ...
    plt.title('Training Data, Experiment 4', fontsize='xx-large')
    ...
    plt.xlim(-0.5, 5.5)
    plt.ylim(-0.5, 20.5)
    plt.savefig('exp4/exp_4_training.png')

    # Train the models
    lrct = LRCTree(max_depth=1, highest_degree=3,
                   n_bins=20).fit(x_train, y_train)
    ...
    split_line = linear_coef * x_test[:, col1] + squared_coef * \
        x_test[:, col1]**2 + cubed_coef * x_test[:, col1]**3 - intercept
    ...
    plt.title('LRCT Learned Decision Boundary, Experiment 4',
              fontsize='xx-large')
    plt.xlabel('Feature 1', fontsize='x-large')
    plt.ylabel('Feature 2', fontsize='x-large')
    plt.xticks(fontsize='large')
    ...
    plt.xlim(-0.5, 5.5)
    plt.ylim(-0.5, 20.5)
    plt.savefig('exp4/exp4_learned.png')
