...
if __name__ == '__main__':
    ...
    np.random.seed(4736)
    two_var_x = np.random.random((5000, 5))
    two_var_x[:, 0] = two_var_x[:, 0] * 5
    two_var_x[:, 1] = two_var_x[:, 1] * 20
    two_var_x[:, 2] = two_var_x[:, 2] * -3
    two_var_x[:, 3] = two_var_x[:, 3] * 17
    two_var_x[:, 4] = two_var_x[:, 4] * -8
    target = (two_var_x[:, 1] > 2 + 2*two_var_x[:, 0] -
              two_var_x[:, 0]**2 + (two_var_x[:, 0]**3)/2).astype(int)
    ...
    for prop in [0.1, 0.2, 0.3]:
        ...
        plt.title(
            f'Training Data, Experiment 8, Noise Level {prop}', fontsize='xx-large')
        ...
        plt.xlim(-0.5, 5.5)
        plt.ylim(-0.5, 20.5)
        plt.savefig(f'exp8/exp_8_training_{prop}.png')

        # Train the models
        lrct = LRCTree(max_depth=1, highest_degree=3,
                       n_bins=20).fit(x_train, y_train_exp)
        ...
        try:
            ...
            split_line = linear_coef * x_test[:, col1] + squared_coef * \
                x_test[:, col1]**2 + cubed_coef * \
                x_test[:, col1]**3 - intercept
            ...
            plt.title(
                f'LRCT Learned Decision Boundary, Experiment 8, Noise Level {prop}', fontsize='xx-large')
            plt.xlabel('Feature 1', fontsize='x-large')
            plt.ylabel('Feature 2', fontsize='x-large')
            plt.xticks(fontsize='large')
            plt.yticks(fontsize='large')
            plt.xlim(-0.5, 5.5)
            plt.ylim(-0.5, 20.5)
            plt.savefig(f'exp8/exp8_learned_{prop}.png')
        except:
            pass
